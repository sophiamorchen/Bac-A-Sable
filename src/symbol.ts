...
// Définition du type FormField qui inclut un label, une valeur, et un type de champ  
type FormField = {
    label: string;                 // Libellé du champ  
    value: any;                   // Valeur du champ (peut être de n'importe quel type)  
    [fieldTypeSymbol]: string;    // Propriété dynamique pour le type de champ  
};
...
const fields: FormField[] = [textField, numberField, dateField]; // Déclaration des champs à valider  
