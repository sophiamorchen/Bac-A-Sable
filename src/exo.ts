...
let tasks: string[] = ["faire les courses", "aller a l'école", "rendre visite a un ami"];
...
function greet1(name: string = "monde") {
    console.log(`Bonjour, ${name}`)
};
greet1('Marco');
...
function callLong(chaine: string): number {
...
let a2: any = "5";
let b2: number = a2 as number; // convertit explicitement 'a' en un nombre en utilisant l'opérateur "as"

console.log(b2); // Affiche 5

// Exemple de casting explicite avec des crochets d'angle <> :
let a1: any = "5";
let b1: number = <number>a1; // converti 'a' en un nombre en utilisant des crochets d'angle < >
...
const nombresx: number[] = [1, 2, 3, 4, 5];

// Calculer la somme des carrés des nombres pairs
const sommeCarresPairs: number = nombres
    .filter((n) => n % 2 === 0)   // [2, 4]
    .map((n) => n * n)            // [4, 16]
